...
(async function FFTModule() {
    async function fftInstantiate(module, imports = {}) {
        ...
        const adaptedExports = Object.setPrototypeOf({
            ...
            getFftBinIndex(freq, samplerate, fftSize) {
                return Math.floor((freq / samplerate) * fftSize);
            },
            getFundamental(real, samplerate, fftSize, harmonicCount) {
                // very broken.
                let l = 0;
                real = real.map(x => { const y = Math.abs(x); l = Math.max(l, y); return y; });
                real = real.map(x => x / l);

                hps = real.slice();
                for (let h = 2; h <= (harmonicCount + 1); h++) {
                    for (let d = 0; d < (real.length / h); d++) {
                        hps[d] *= real[d * h] || 0;
                    }
                }
                let newMax = 0;
                hps.forEach(x => { newMax = Math.max(newMax, x); });
                const fundamentalBinIndex = hps.indexOf(newMax);
                const freq = (fundamentalBinIndex / fftSize) * samplerate;
