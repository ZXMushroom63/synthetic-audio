...
const hvcc2functor = function (compiledModule, meta) {
    return async function (inPcm, channel, data) {
        ...
        return offlineContext.startRendering().then(renderedBuffer => {
            return renderedBuffer.getChannelData(0).subarray(HVCC_PROCESSOR_BLOCKSIZE*HVCC_PROCESSOR_PACKET_OFFSET).map(x => x*2); //there is a delay equal to the blocksize as the processing is done asynchronously :p
        });
    }
}