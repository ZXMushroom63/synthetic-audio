    ...
    if (typeof val === "string" && val.startsWith("#")) {
        if (val.split("~").length === 2) {
            ...
            if (upscaleSize && waveformFn) {
                ...
                var upscaleFactor = waveformFn.length / upscaleSize;
                var finalValue = waveformFn[waveformFn.length - 1];
                waveformUpscaled.forEach((x, i) => {
                    var idx = i * upscaleFactor;
                    var k = (i % (1 / upscaleFactor)) * upscaleFactor;
                    var end = waveformFn[Math.ceil(idx)];

                    waveformUpscaled[i] = lerp(
                        waveformFn[Math.floor(idx)],
                        isNaN(end) ? finalValue : end,
                        k
                    );
                });
                waveformFn = waveformUpscaled; //todo: fix last upscale block being nothing but last value
            }

            if (waveformFn) {
                if (waveformHz !== 0) {
                    var outFn = (x, pcm) => {
                        return lerp(v[0], v[1], (waveformFn[Math.round(x / audio.samplerate * waveformHz * waveformFn.length) % waveformFn.length] + 1) / 2);
                    }
                    outFn.LFOType = "waveform";
                    return outFn;
                }
                var outFn = (x, pcm) => {
                    return lerp(v[0], v[1], (waveformFn[Math.round(x / pcm.length * waveformFn.length)] + 1) / 2);
                }
                outFn.LFOType = "waveform";
                return outFn;
            }
