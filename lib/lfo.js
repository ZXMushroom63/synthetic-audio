    ...
    if (typeof val === "string" && val.startsWith("#")) {
        if (val.split("~").length === 2) {
            ...
            if (waveformFn) {
                if (waveformHz !== 0) {
                    return (x, pcm) => {
                        return lerp(v[0], v[1], (-1*waveformFn[Math.floor(x / audio.samplerate * waveformHz * waveformFn.length) % waveformFn.length] + 1) / 2);
                    }
                }
                return (x, pcm) => {
                    return lerp(v[0], v[1], (-1*waveformFn[Math.floor(x / pcm.length * waveformFn.length)] + 1) / 2);
                }
            }
            return (x, pcm) => {
