...
function _(val, cfg) {
    cfg = cfg || {};
    cfg.upscaleSize ||= null; // upscale waveform functions
    cfg.disallowAutomationParams ||= false;
    if (typeof val === "string" && val.includes(":")) {
        ...
        noteArray = noteArray.map(note => {
            ...
            var microtone = 0;
            const toneBase = note.split("⁺")[1] || note.split("⁻")[1];
            if (toneBase) {
                microtone = parseInt(fromSuperscript(toneBase));
                if (note.includes("⁻")) {
                    microtone *= -1;
                }
                microtone *= 0.1;
            ...
            switch (note.length) {
                case 1:
                    return noteToFrequency(note, 4, "", microtone);
                case 2:
                    if (VALID_DESCRIPTORS.includes(note[1])) {
                        return noteToFrequency(note[0], 4, note[1], microtone);
                    } else {
                        return noteToFrequency(note[0], parseInt(note[1]), "", microtone);
                    }
                case 3:
                    return noteToFrequency(note[0], parseInt(note[2]), note[1], microtone);
            }
        });
        val = arr.map((x, i) => {
            if (i % 2) {
    ...
    if (typeof val === "string" && val.startsWith("@")) {
        if (cfg.disallowAutomationParams) {
            return () => 0;
        }
        const loopOffset = Math.round((currentlyRenderedLoop?.start || 0) * audio.samplerate);
        const parts = val.replace("@", "").split("!");
    ...
    if (typeof val === "string" && val.startsWith("#")) {
        if (val.split("~").length === 2) {
            ...
            var v = val.replace("#", "").split("~").map((x, i) => {
                if (i === 0) {
            ...
            const upscaleSize = cfg.upscaleSize;
            if (upscaleSize && waveformFn) {
                const waveformUpscaled = new Float32Array(upscaleSize);
                const upscaleFactor = (waveformFn.length - 1) / upscaleSize;
                const finalValue = waveformFn[waveformFn.length - 1];
            ...
            if (exponent === 1) {
                outFn = (x, pcm) => {
                    return lerp(v[0], v[1], x / (pcm.length - 1));
                }
            } else {
                outFn = (x, pcm) => {
