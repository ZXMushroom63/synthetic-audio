...
function _(val, upscaleSize) {
    ...
    if (typeof val === "string" && val.startsWith("@")) {
        if (!currentlyRenderedLoop) {
            return ()=>0.5;
        }
        const loopOffset = (currentlyRenderedLoop.start) * audio.samplerate;
        const parts = val.replace("@", "").split("!");
        const internalId = "@__params::" + parts[0].toUpperCase();
        if (!proceduralAssets.has(internalId)) {
            return () => 0;
        }
        const paramPcm = proceduralAssets.get(internalId);
        debugger;
        if (parts.length === 2) {
            const targetMapping = parts[1].split("~").map(x => parseFloat(x.trim()));
            return (i, pcm) => {
                return lerp(targetMapping[0], targetMapping[1], paramPcm[loopOffset + i] || 0) || 0;
            }
        } else {
            return (i, pcm) => {
...
waveformsDropdown.addEventListener("mousedown", (e) => {
    ...
    e.preventDefault();
    e.stopPropagation();
    if (e.target.tagName === "LI" && lastEditedField && lastEditedField.value.startsWith("@")) {
