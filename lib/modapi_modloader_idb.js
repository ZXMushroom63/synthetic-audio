...
globalThis.getSample = async function getSample(mod) {
    const db = await getDatabase();
    const transaction = db.transaction(["filesystem"], "readonly");
    const objectStore = transaction.objectStore("filesystem");
    const object = await promisifyIDBRequest(objectStore.get("samples/" + mod));
    var out = object ? (object) : null;
    db.close();
    return out;
}
...
globalThis.addSample = async function addSample(name, blob) {
    const mods = await getMods();
    if (mods.includes(name)) {
        await removeMod(mods.indexOf(name));
    } else {
        mods.push(name);
    }
    await saveMods(mods);
    ...
    const transaction = db.transaction(["filesystem"], "readwrite");
    const objectStore = transaction.objectStore("filesystem");
    await promisifyIDBRequest(objectStore.put(blob, "samples/" + name));
    db.close();
}
...
globalThis.removeMod = async function removeMod(index) {
    ...
    if (index >= 0 && index < mods.length) {
        ...
        if (!deleted.startsWith("web@")) {
            ...
            await promisifyIDBRequest(objectStore.delete("samples/" + deleted));
            db.close();
        }
    }
}
