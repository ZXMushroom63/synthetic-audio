...
function ModMenuMarkedTabList() {
    ...
    this.addTab = function (name, markdown) {
        this._tabs[name] = marked.parse(markdown.replaceAll("~~~", "```").replaceAll(" ~", " `").replaceAll("~ ", "` ").replaceAll("~\n", "`\n").replaceAll("\n~", "\n`"), { renderer: this.renderer });
    };
}
...
function ModMenu(title, tabs, rootDivId = "menu", style = undefined) {
    ...
    this._dragElement = function (elmnt) {
      ...
      /*/ calculate the new cursor position:/*/ pos1 =
            ...
            elmnt.style.top =
                `min(max(${
                    elmnt.style.height ? `calc(${elmnt.style.height} / 2)` : "25vh"
                }, ${elmnt.offsetTop - pos2}px), ${
                    elmnt.style.height ? `calc(100vh - ${elmnt.style.height} / 2)` : "75vh"
                })`;
            elmnt.style.left = `min(max(${
                elmnt.style.width ? `calc(${elmnt.style.width} / 2)` : "25vw"
            }, ${elmnt.offsetLeft - pos1}px), ${
                elmnt.style.width ? `calc(100vw - ${elmnt.style.width} / 2)` : "75vw"
            })`;
        }
    ...
    this._parseTabsBar = function (tabs) {
        ...
        for (let i = 0; i < tabNames.length; i++) {
            ...
            html += `<button id="modmenu_${name}_btn" data-tabname="${name}" class="modmenutablinks ${i === 0 ? "active" : ""}">${name}</button>`;
        }
    ...
    this.init = function (instanceOpts) {
        ...
        this.rootDiv.querySelector(`#${this.id}header a`).addEventListener("mousedown", (e) => {
            e.stopPropagation();
        ...
        this.rootDiv.addEventListener("click", (event) => {
            if (event.target.closest(".modmenutablinks")) {
                this.openTab(event, event.target.closest(".modmenutablinks").getAttribute("data-tabname"));
            }
        })
        this.rootDiv.classList.add("__modmenu_div");
        this.openTab = function MODMENU_OpenTab(unused, cityName) {
        ...
        this._injectStyle(
            `#${rootDivId} { all:initial; color: ${this.style._style.textColor}; user-select: none; font-family: sans-serif; position: fixed; z-index: 1999; background-color: ${this.style._style.background}; border: 1px solid #d3d3d3; width:${this.style._style.width}; height:${this.style._style.height}; resize:both; overflow-y:scroll; top: 50%; left:50%; transform: translate(-50%, -50%); border-radius: 0.5rem; box-shadow: 2px 2px 52px 0px rgba(0,0,0,1)}
            #${rootDivId}header { padding: 10px; color: ${this.style._style.textColor}; cursor: move; z-index: 2000; background-color: ${this.style._style.headerBackground}; color: ${this.style._style.headerText}; text-align: center; }
