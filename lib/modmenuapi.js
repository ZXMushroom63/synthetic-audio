...
function ModMenu(title, tabs, rootDivId = "menu", style = undefined) {
    ...
    this._parseTabsBar = function (tabs) {
        ...
        for (let i = 0; i < tabNames.length; i++) {
            ...
            html += `<button class="modmenutablinks ${i === 0 ? "active" : ""}" onclick="MODMENU_OpenTab(event, '${name}');">${name}</button>`;
        }
    ...
    this._parseTabsContent = function (tabs) {
        ...
        for (let i = 0; i < tabNames.length; i++) {
            ...
            html += `<div id="${name}" class="modmenutabcontent" ${i === 0 ? "style=\"display:block\"" : ""}>${content}</div>`;
        }
        return html;
    };
    ...
    this.init = function () {
        ...
        this.rootDiv.querySelector(`#${this.id}header a`).addEventListener("click", ()=>{
            this.closeModMenu();
        });
        ...
        this._injectStyle(
            ` #${rootDivId} { all:initial; color: ${this.style._style.textColor}; user-select: none; font-family: sans-serif; position: fixed; z-index: 89999999; background-color: ${this.style._style.background}; border: 1px solid #d3d3d3; width:${this.style._style.width}; height:${this.style._style.height}; resize:both; overflow-y:scroll; top:0; left:0; border-radius: 0.5rem; box-shadow: 2px 2px 52px 0px rgba(0,0,0,1)}
            ...
            #${rootDivId} .modmenutabcontent { color: ${this.style._style.textColor}; display: none; padding: 6px 12px; }`
        );
