...
addBlockType("same", {
    ...
    title: "SAME Encoder (EAS)",
    credits: "https://github.com/aaron-em/same-encoder",
    configs: {
        "Volume": [1, "number"],
        "Sender": ["8MAXLENT", "text"],
        "Origin": ["EAS/Emergency Alert System", ["EAS/Emergency Alert System", "CIV/Civil Authorities", "WXR/National Weather Service", "PEP/Primary Entry Point System"]],
        "Event": ["EVI/Evacuation Immediate", ['EAN/Emergency Action Notification', 'EAT/Emergency Action Termination', 'NIC/National Information Center', 'NPT/National Periodic Test', 'RMT/Required Monthly Test', 'RWT/Required Weekly Test', 'ADR/Administrative Message', 'AVW/Avalanche Warning', 'AVA/Avalanche Watch', 'BZW/Blizzard Warning', 'CAE/Child Abduction Emergency', 'CDW/Civil Danger Warning', 'CEM/Civil Emergency Message', 'CFW/Coastal Flood Warning', 'CFA/Coastal Flood Watch', 'DSW/Dust Storm Warning', 'EQW/Earthquake Warning', 'EVI/Evacuation Immediate', 'FRW/Fire Warning', 'FFW/Flash Flood Warning', 'FFA/Flash Flood Watch', 'FFS/Flash Flood Statement', 'FLW/Flood Warning', 'FLA/Flood Watch', 'FLS/Flood Statement', 'HMW/Hazardous Materials Warning', 'HWW/High Wind Warning', 'HWA/High Wind Watch', 'HUW/Hurricane Warning', 'HUA/Hurricane Watch', 'HLS/Hurricane Statement', 'LEW/Law Enforcement Warning', 'LAE/Local Area Emergency', 'NMN/Network Message Notification', 'TOE/911 Telephone Outage Emergency', 'NUW/Nuclear Power Plant Warning', 'DMO/Practice/Demo Warning', 'RHW/Radiological Hazard Warning', 'SVR/Severe Thunderstorm Warning', 'SVA/Severe Thunderstorm Watch', 'SVS/Severe Weather Statement', 'SPW/Shelter in Place Warning', 'SMW/Special Marine Warning', 'SPS/Special Weather Statement', 'TOR/Tornado Warning', 'TOA/Tornado Watch', 'TRW/Tropical Storm Warning', 'TRA/Tropical Storm Watch', 'TSW/Tsunami Warning', 'TSA/Tsunami Watch', 'VOW/Volcano Warning', 'WSW/Winter Storm Warning', 'WSA/Winter Storm Watch']],
        "State": ['00/Entire nation', ['00/Entire nation', '10/DE', '11/DC', '12/FL', '13/GA', '15/HI', '16/ID', '17/IL', '18/IN', '19/IA', '20/KS', '21/KY', '22/LA', '23/ME', '24/MD', '25/MA', '26/MI', '27/MN', '28/MS', '29/MO', '30/MT', '31/NE', '32/NV', '33/NH', '34/NJ', '35/NM', '36/NY', '37/NC', '38/ND', '39/OH', '40/OK', '41/OR', '42/PA', '44/RI', '45/SC', '46/SD', '47/TN', '48/TX', '49/UT', '50/VT', '51/VA', '53/WA', '54/WV', '55/WI', '56/WY', '60/AS', '66/GU', '69/MP', '72/PR', '74/UM', '78/VI', '01/AL', '02/AK', '04/AZ', '05/AR', '06/CA', '08/CO', '09/CT']],
    ...
    selectMiddleware: function (key) {
        if (key === "County") {
            return ["000/Entire state", ...Object.entries(sameCounties[this.conf.State.split("/")[0]] || {}).map(ent => ent[0] + "/" + ent[1])];
        }
    },
    hidden: false,
    waterfall: 1,
    functor: async function (inPcm, channel, data) {
        const offlineContext = new OfflineAudioContext(1, inPcm.length, audio.samplerate);
        const date = new Date(this.conf.StartTime);
        const startOfYear = new Date(date.getFullYear(), 0, 1);
        const delta = date.getTime() - startOfYear.getTime();
        const day = Math.floor(delta / 86400000);

        const msg = {
            sender: this.conf.Sender.slice(0, 8).replaceAll("-", "_"),
            originator: this.conf.Origin.split("/")[0],
            code: this.conf.Event.split("/")[0],
            region: {
                subdiv: this.conf.Division.split("/")[0],
                stateCode: this.conf.State.split("/")[0],
                countyCode: this.conf.County.split("/")[0]
            },
            length: parseInt(this.conf.PurgeTimeHH_MM),
            start: {
                day: day + 1,
        ...
        const buf = await (await fetch('data:audio/wav;base64,' + escape(btoa(SAME.Encoder.encode(msg))))).arrayBuffer();
        const sameHeaderPcm = (await offlineContext.decodeAudioData(buf)).getChannelData(0);
        const len = Math.min(sameHeaderPcm.length, inPcm.length);
        for (let i = 0; i < len; i++) {
            inPcm[i] += sameHeaderPcm[i] * this.conf.Volume;                
        }
        return inPcm;
    }
});