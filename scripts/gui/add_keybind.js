...
function calcOptions(search) {
    ...
    var opts = Object.entries(filters)
        ...
        .filter(x => x[0] !== "audio").map(x => {
            ...
            ret._key = x;
            return ret
        }).sort().sort((a, b)=>{return (b.toLowerCase().startsWith(search)) - (a.toLowerCase().startsWith(search))}).sort((a, b)=>{return (b.toLowerCase() === search) - (a.toLowerCase() === search)}).map(x => x._key)
        .map(x => [x[0], x[1].title])
